/**
 * Shared Semantic Positioning Library for Frontend
 *
 * Provides intelligent node positioning for workflow diagrams using semantic grouping
 * and virtual column/row layouts. Shared between AI components to avoid duplication.
 */
...
export const DEFAULT_LAYOUT_CONFIG: LayoutConfig = {
  ...
  rowHeight: 110,        // Tighter vertical spacing
...
/**
 * Convenience function for quick positioning - legacy support for simpler cases
 */
...
/**
 * Legacy positioning function for backward compatibility with existing IntegratedAIAssistant
 */
export function getBestPositionForNewNode(scenario: any, targetNodeId?: string): Position {
  const nodes: Node[] = Array.isArray(scenario?.nodes) ? scenario.nodes : [];
  const edges: Edge[] = Array.isArray(scenario?.edges) ? scenario.edges : [];

  if (targetNodeId) {
    const target = nodes.find(n => n.nodeId === targetNodeId);
    if (target?.position) {
      // Use semantic positioning for more intelligent placement
      const positioner = new SemanticPositioner();
      const positioned = positioner.positionNewNodes([target], [{ nodeId: 'temp', type: 'Node' }], edges);
      return positioned[0]?.position || { x: target.position.x + 180, y: target.position.y };
    }
  }
...
/**
 * Legacy collision fixing for backward compatibility
 */
export function fixCollisionsInPlace(scenario: any, existingNodeIds: Set<string> = new Set()): void {
  ...
  const existingNodes = nodes.filter(n => existingNodeIds.has(n.nodeId));
  const newNodes = nodes.filter(n => !existingNodeIds.has(n.nodeId));

  if (newNodes.length === 0) return;

  const positioner = new SemanticPositioner();
  const positioned = positioner.positionNewNodes(existingNodes, newNodes, edges);

  // Apply new positions back to the scenario
  positioned.forEach(positionedNode => {
    const originalNode = nodes.find(n => n.nodeId === positionedNode.nodeId);
    if (originalNode) {
      originalNode.position = positionedNode.position;
    }
  });
}