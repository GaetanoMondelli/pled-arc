  ...
  try {
    ...
    if (!tokenId || !walletAddress) {
      return NextResponse.json(
        { error: "Token ID and wallet address required" },
        { status: 400 }
      );
    }
    ...
    if (!arcWallet) {
      return NextResponse.json(
        { error: "No ARC-TESTNET wallet found" },
        { status: 500 }
      );
    }
    ...
    // Wait for transaction confirmation (simplified - in production, use webhooks)
    let attempts = 0;
    ...
    while (attempts < maxAttempts) {
      ...
      if (txStatus === "COMPLETE") {
        // Get the reward amount from events
        return NextResponse.json({
          success: true,
          transactionId: txId,
