/**
 * Deploy GreenEnergyReward contract to Arc using Circle Smart Contract Platform SDK
 */
...
async function deployGreenEnergyReward() {
  ...
  const deployment = await scpClient.deployContract({
    name: 'GreenEnergyReward',
    description: 'Green Energy Carbon Credit Fund Reward - Escrow for USDC rewards based on verifiable claims',
  ...
  while (attempts < maxAttempts) {
    ...
    if (contractData?.deployStatus === 'DEPLOYED') {
      ...
      console.log(`NEXT_PUBLIC_GREEN_ENERGY_REWARD_ADDRESS=${contractData.address}\n`);
      console.log('\n💡 Next steps:');
